...
The backtesting system is built around a generic, modular architecture:

**Core Components**:
- **`Backtester`**: Generic backtest runner that works with any strategy
...
The generic backtest system makes adding new strategies straightforward:
...
3. **Add Run Function**:
   ```rust
   async fn run_my_new_strategy(mode: TradingMode, initial_capital: f64) -> Result<()> {
       match mode {
           TradingMode::Backtest => {
               let config = BacktestConfig {
                   initial_capital,
                   assets: vec!["ASSET1".to_string(), "ASSET2".to_string()],
                   strategy_name: "My New Strategy".to_string(),
                   ..BacktestConfig::default()
               };
               
               let results = Backtester::run(
                   |capital| MyNewStrategy::new(capital),
                   config.clone(),
               ).await?;
...
4. **Update Main Match Statement**:
   ...
   match args.strategy {
       StrategyType::MyNewStrategy => {
           run_my_new_strategy(args.mode, args.capital).await?;
       },
       // ... other strategies
   }
   ```
...
- **No Code Duplication**: Backtest logic is centralized
- **Consistent Metrics**: All strategies get the same performance analysis
- **Easy Testing**: Generic backtester is thoroughly tested
- **Extensible**: Add new metrics without touching strategy code
